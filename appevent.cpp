...
//        }
            ...
            cv::cvtColor(frame2, frame2, cv::COLOR_BGR2HSV);

        }
        else if(eventType == BEvent){
            frame2=displayBchannel(frame2);
        }
        else if(eventType == GEvent){
            frame2=displayGchannel(frame2);
        }
        else if(eventType == REvent){
            frame2=displayRchannel(frame2);
        }
        else if(eventType == BinthEvent){
            cv::threshold(frame2,frame2,m_val,255,cv::THRESH_BINARY);
        }
        else if(eventType == BinthinvEvent){
            cv::threshold(frame2,frame2,m_val,255,cv::THRESH_BINARY_INV);
        }
        else if(eventType == ThtrEvent){
            cv::threshold(frame2,frame2,m_val,255,cv::THRESH_TRUNC);
        }
        else if(eventType == ThtoinvEvent){
            cv::threshold(frame2,frame2,m_val,255,cv::THRESH_TOZERO_INV);
        }
        else if(eventType == ThtoEvent){
            cv::threshold(frame2,frame2,m_val,255,cv::THRESH_TOZERO);
        }
        else if(eventType == adThEvent){
            cv::cvtColor(frame2, frame2, cv::COLOR_BGR2GRAY);
            cv::adaptiveThreshold(frame2,frame2,255,cv::ADAPTIVE_THRESH_GAUSSIAN_C,cv::THRESH_BINARY,5,3);
        }
        else if(eventType == avblEvent){
            cv::blur(frame2, frame2, cv::Size(5,5) );
        }
        else if(eventType == bofilEvent){
            cv::boxFilter(frame2, frame2, -1, cv::Size(5,5));
        }
        else if(eventType == gablEvent){
            cv::GaussianBlur(frame2, frame2, cv::Size(5,5),0,0);
        }
            ...
            cv::bilateralFilter(frame,frame2,9,m_val,m_val);
        }
...
//            cv::Mat kernel = (Mat_<char>(3, 3) << 1, 1, 1, 1, -8, 1, 1, 1, 1);
            ...
            cv::filter2D(frame2, frame2, CV_8UC3, kernel);
        }
    }
